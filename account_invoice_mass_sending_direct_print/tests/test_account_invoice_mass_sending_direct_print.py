...
class TestAccountInvoiceMassSendingDirectPrint(TransactionCase):
    ...
    def test_mass_sending_with_printer_mock(self):
        ...
        self.invoice.write({"sending_in_progress": False})
        with patch(
            "odoo.addons.base_report_to_printer.models.ir_actions_report.IrActionsReport._render_qweb_pdf"
        ) as mock_render_pdf:
            wizard = (
                self.env["account.invoice.send"]
                .with_context(active_ids=self.invoice.ids)
    ...
    def test_render_qweb_pdf_with_direct_print(self):
        ...
        with patch(
            ...
            # Change the action type of the printing action to client to not access `print_document`
